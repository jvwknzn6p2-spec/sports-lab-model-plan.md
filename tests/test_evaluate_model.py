...
asserted exactly. The script is called in-process through `main(argv)`.
"""
...
def run(inp: Path, out: Path, *extra: str) -> tuple[int, dict]:
    code = ev.main(["--input", str(inp), "--output", str(out), "--bootstrap-samples", "300", *extra])
    return code, json.loads(out.read_text(encoding="utf-8"))


def evaluate(tmp_path: Path, rows: list[dict], *extra: str) -> tuple[int, dict]:
    return run(write_csv(tmp_path / "in.csv", rows), tmp_path / "out.json", *extra)
...
def test_push_rows_are_excluded_and_counted(tmp_path: Path):
    ...
    assert set(rep["overall"]["candidate"]) == {"brier", "log_loss", "ece_10bin", "accuracy_0_5"}
...
def test_statistically_worse_candidate_regresses_above_gate_minimum(tmp_path: Path):
    ...
    code, rep = evaluate(tmp_path, rows, "--min-gate-samples", "200")
    assert code == 3
...
def test_deterministic_given_seed(tmp_path: Path):
    inp = write_csv(tmp_path / "in.csv", synthetic_rows(250, 9, 0.1, 0.1))
    _, a = run(inp, tmp_path / "a.json", "--seed", "11")
    _, b = run(inp, tmp_path / "b.json", "--seed", "11")
    assert a["overall"]["comparison"] == b["overall"]["comparison"]


@pytest.mark.parametrize("label,expected_accuracy", [("WIN", 1.0), ("LOSS", 0.0), ("W", 1.0), ("L", 0.0)])
def test_settlement_result_labels(tmp_path: Path, label: str, expected_accuracy: float):
    # One row at 0.9 → predicts WIN; accuracy is 1 exactly when the label reads as a win.
    rows = synthetic_rows(1, 10, 0.05, 0.05)
    rows[0].update(actual_outcome="", settlement_result=label, candidate_prob="0.9")
    code, rep = evaluate(tmp_path, rows)
    assert code == 0
