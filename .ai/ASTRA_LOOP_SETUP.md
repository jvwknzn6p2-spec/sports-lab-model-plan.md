...
Claude Code ──► PR (feature branch, never main)
   ...
   .github/workflows/ai-development-loop.yml   (read-only job)
     1. pnpm typecheck + lib/sports-data tests + lib/football-model tests + pytest
     2. scripts/export_evaluation.py  → data/evaluation/predictions_eval.csv (+ manifest)
     3. scripts/evaluate_model.py     → reports/latest_evaluation.json
...
`ci.yml` keeps running unchanged alongside; the loop adds checks, it replaces
none.
...
| `AI_LOOP_PAT` | repository **secret** | optional, only when `AUTO_CLAUDE_FIX=true`. A fine-grained PAT of the Founder's account with `Issues: write` and `Pull requests: write` on this repository | `ai-development-loop.yml` (repair-request comment) |
...
- Evidence is regenerated on every run from the committed ledgers; the CSV
  and JSON are uploaded as the workflow artifact `astra-evidence-<run id>`
  (30 days) and are git-ignored.
